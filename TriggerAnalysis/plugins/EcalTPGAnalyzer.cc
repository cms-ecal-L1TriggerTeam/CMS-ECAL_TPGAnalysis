...
//        Numbers::initGeometry(iSetup, false);
    ...
    for ( int itBit = 0; itBit < 128; ++itBit ) { 
      //      std::cout<<"bucho wucho re"<<std::endl;
      iBit++;
      int fired=dWord[iBit];
      //      std::cout<<"bool "<<triggerMaskAlgoTrig[iBit]<<std::endl;
      int maskBit = triggerMaskAlgoTrig[iBit] & (1 << iDaq);
      //      std::cout<<"maskbit "<<maskBit<<endl;
      if (maskBit) fired  = 0; //else std::cout<<"not********** masked********"<<std::endl;
      if (fired) {
	treeVariables_.activeTriggers[treeVariables_.nbOfActiveTriggers] = iBit ;
	treeVariables_.nbOfActiveTriggers++ ;      
	//	std::cout<<"fire koreche bhai"<<std::endl;
	//	std::cout<<"ibit "<<iBit<<std::endl;
      }

    }
    //    std::cout<<"etto gula aase  "<<treeVariables_.nbOfActiveTriggers<<std::endl;
